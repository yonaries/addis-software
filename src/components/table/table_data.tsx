    ...
    <>
      <table className="w-full text-center">
        ...
        <tbody className="text-slate-50 px-5">
          {list.map((user: IUser, index) => (
            <tr
              onDoubleClick={() => viewHandler(user.id!)}
              className="h-11 cursor-pointer hover:bg-zinc-700 transition-all"
              key={user.id}
            >
              ...
              <td className="flex justify-evenly pt-2">
                <img
                  className="border-2 border-transparent rounded-md hover:bg-zinc-800 transition-all"
                  alt="edit record"
                  src={editIcon}
