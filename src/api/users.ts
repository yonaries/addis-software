...
export const createRecord = async (record: IUser) => {
  try {
    await axios.post(
      `http://127.0.0.1:8090/api/collections/users_data/records`,
      record
    );
  } catch (error: any) {
...
export const fetchRecords = async (): Promise<IUser[]> => {
  try {
    const result = await axios.get(
      "http://127.0.0.1:8090/api/collections/users_data/records?sort=-created"
    );
...
export const searchRecords = async (keyword: string): Promise<IUser[]> => {
  try {
    ...
    const result = await axios.get(
      `http://127.0.0.1:8090/api/collections/users_data/records?filter=(full_name~'${keyword}'||user_name~'${keyword}'||email~'${keyword}')`
    );
...
export const checkUsername = async (username: string) => {
  try {
    const result = await axios.get(
      `http://127.0.0.1:8090/api/collections/users_data/records?filter=(user_name='${username}')`
    );
